...
async function readAll() { await accountApi.readAllNotifications(); notices.value.forEach(item => { item.read = true }); unread.value = 0 }
...
<template>
  <div class="account-page">
    <header class="account-nav"><router-link to="/coach" class="brand"><span>PQ</span><strong>PolicyQuest</strong></router-link><UserAccountMenu /></header>
    <main>
      <div class="breadcrumbs"><button type="button" @click="router.push('/coach')">学习中心</button><span>/</span><b>{{ current.title }}</b></div>
      <section class="account-hero"><div><small>ACCOUNT CENTER</small><h1>{{ current.title }}</h1><p>{{ current.subtitle }}</p></div></section>
      <div class="account-grid">
        <aside><router-link to="/account/orders" :class="{ active: section === 'orders' }">订单管理 <em>2.0</em></router-link><router-link to="/profile">个人信息</router-link><router-link to="/account/feedback" :class="{ active: section === 'feedback' }">反馈和建议</router-link><router-link to="/account/guide" :class="{ active: section === 'guide' }">使用步骤</router-link><router-link to="/account/notifications" :class="{ active: section === 'notifications' }">消息通知 <i v-if="unread">{{ unread }}</i></router-link></aside>
