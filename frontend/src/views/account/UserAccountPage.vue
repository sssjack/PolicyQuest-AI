    ...
    <main>
      ...
      <div class="account-grid">
        ...
        <section class="content-card">
          ...
          <template v-else-if="section === 'notifications'"><div class="content-title"><div><h2>消息通知</h2><p>{{ unread ? `${unread} 条未读消息` : '全部消息均已读' }}</p></div><el-button v-if="unread" link type="primary" @click="readAll">全部已读</el-button></div><div v-if="!notices.length" class="empty">暂无通知公告</div><article v-for="item in notices" :key="item.id" class="notice-row" :class="{ unread: !item.read }" @click="read(item)" role="button" tabindex="0" @keydown.enter="read(item)" @keydown.space.prevent="read(item)"><i></i><div><header><b>{{ item.title }}</b><small>{{ date(item.publishedAt) }}</small></header><p>{{ item.content }}</p></div></article><article class="notice-row order-notice" @click="read(orderNotice)" role="button" tabindex="0" @keydown.enter="read(orderNotice)" @keydown.space.prevent="read(orderNotice)"><i></i><div><header><b>订单状态 <em>2.0</em></b><small>功能预告</small></header><p>订单进度、支付状态与退款通知将在 2.0 版本接入。</p></div></article></template>
