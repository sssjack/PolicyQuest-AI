...
<template>
  <div v-if="userStore.isLoggedIn" class="account-actions">
    ...
    <el-popover placement="bottom-end" :width="370" trigger="click" popper-class="pq-notice-popover" @show="loadNotifications">
      ...
      <div class="notice-panel"><header><div><b>消息通知</b><small>{{ unread ? `${unread} 条未读` : '暂无未读消息' }}</small></div><button v-if="unread" type="button" @click="readAll">全部已读</button></header><button class="notice-item deferred" type="button" @click="router.push(target('/account/orders'))"><i></i><span><b>订单状态 <em>2.0</em></b><small>订单进度与支付状态将在 2.0 版本接入。</small></span></button><div v-if="loading" class="notice-empty">正在加载…</div><div v-else-if="!notices.length" class="notice-empty">暂无通知公告</div><button v-for="item in notices" v-else :key="item.id" class="notice-item" :class="{ unread: !item.read }" type="button" @click="readNotice(item)"><i></i><span><b>{{ item.title }}</b><small>{{ item.content }}</small></span></button><footer><button type="button" @click="router.push(target('/account/notifications'))">查看全部通知</button></footer></div>
    </el-popover>
