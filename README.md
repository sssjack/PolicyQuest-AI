...
- **交互首页**：蓝色玻璃轨道主视觉与轻量训练入口，提供小题／作文评分展示、材料采点、逐句改写和答题拆解演示；示例均明确标注为教学展示，支持手机访问。
...
- **整卷能力诊断**：每套申论全部批改后自动汇总成绩、十维能力、跨题共性问题、优势、每题复盘、错误趋势、提分优先级、目标分差和七天计划；支持历史报告补生成与总结独立重试。[完整说明](docs/ESSAY_PAPER_REPORT.md)
...
- **后台管理**：支持题库、文章源、AI 任务、用户状态与用户积分调整；每次调分保留原因和操作人。
- **积分与手写答题**：新注册赠100积分，每套真题提交扣10积分；支持 PaddleOCR 中文手写照片识别、校对改字、分段排序和追加答案。[使用与部署](docs/CREDITS_HANDWRITING.md)
...
测试文件仅保留本地，不纳入 Git 或发布包；后端当前的 `npm test` 是占位脚本。已完成11项后端回归测试、报告服务端渲染检查和生产真实模型批改验证。历史报告修复另通过9组浏览器检查：使用线上构建资源和模拟接口数据，覆盖旧版报告、新版报告、失败状态及1920/1024/390像素宽度，验证列对齐、无横向溢出和原题/材料/答案切换；这不替代完整登录提交链路的端到端验收。
